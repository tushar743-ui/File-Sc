...
def test_trailing_comment_suppresses(run_scan, shipped_rules):
    source = build(suffix="  # taintscan: ignore[py.sql-injection] name is allowlisted")
    assert run_scan({"app.py": source}, shipped_rules) == []


def test_legacy_marker_also_suppresses(run_scan, shipped_rules):
...
def test_comment_on_the_line_above_suppresses(run_scan, shipped_rules):
    source = build(above="# taintscan: ignore[py.sql-injection] name is allowlisted")
    assert run_scan({"app.py": source}, shipped_rules) == []


def test_bare_ignore_suppresses_every_rule(run_scan, shipped_rules):
    source = build(suffix="  # taintscan: ignore reviewed by security 2026-09")
    assert run_scan({"app.py": source}, shipped_rules) == []


def test_other_rule_id_does_not_suppress(run_scan, shipped_rules):
    source = build(suffix="  # taintscan: ignore[py.ssrf] wrong rule")
...
def test_suppression_without_a_reason_is_itself_reported(run_scan, shipped_rules):
    source = build(suffix="  # taintscan: ignore[py.sql-injection]")
...
def test_reasoned_suppression_produces_no_meta_finding(run_scan, shipped_rules):
    source = build(suffix="  # taintscan: ignore[py.sql-injection] checked against an allowlist")
    assert run_scan({"app.py": source}, shipped_rules) == []
